...
fukashima_sent_summary <- fukashima_sent_df %>%
  group_by(Par_ID) %>%
  summarize(sentiment_average = mean(sentiment))

fukashima_sent_summary_date <- inner_join(fukashima_sent_summary, fukashima_sent_df, by = "Par_ID") %>% 
  select(Par_ID, sentiment_average, Date)

# check to see if any NA values are present in the df
apply(is.na(fukashima_sent_summary_date), 2, sum) # the argument "2" tells the function to sum over columns
...
fukushima_pos_neg_neutral <- fukashima_sent_summary_date %>% 
  mutate(sent_category = case_when(
...
ggplot(fukushima_pos_neg_neutral, aes(x = Date, y = n)) +
  ...
  scale_color_manual(values = c("red", "deepskyblue4", "forestgreen")) +
  ...
  labs(x = "Date of Sentiment",
       y = "Number of Paragraphs with Sentiment",
